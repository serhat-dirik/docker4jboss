...
if  ! $CSTATE  ;  then   docker run --privileged --name docker-dns --hostname docker-dns -d -v /var/run/docker.sock:/docker.sock  phensley/docker-dns  --domain docker; fi
...
for container in mariadb-server postgres-server dv6-server; do
